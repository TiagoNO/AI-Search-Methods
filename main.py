...
from ucs import ucs_search,write_arq
from bfs import calculate_heuristic,bfs_search
...
if str(sys.argv[1]) == 'ids':
    ...
    while not found:
        map_copy = input.map
        state = input.initial_state + [0.0,1,[-1,-1]]
        open_list = {0:state}
        map_list = {(state[0],state[1]):0}
        closed_list = {}
        found = ids_search(map_copy,input.map_height,input.map_width,open_list,map_list,closed_list,input.goal_state,lim)
        lim += 1
...
#    write_arq("ucs_log",input.map)
    ...
    calculate_heuristic(input.map,input.map_height,input.map_width,heuristic_map,input.goal_state)

    state = [heuristic_map[(input.initial_state[0],input.initial_state[1])],(input.initial_state[0],input.initial_state[1]),(-1,-1),0.0]
    heap = [state]
    bfs_search(input.map,input.map_height,input.map_width,heap,heap_map,heuristic_map,input.goal_state)
